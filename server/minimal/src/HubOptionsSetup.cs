...
// https://github.com/dotnet/aspnetcore/blob/main/src/SignalR/server/Core/src/HubOptionsSetup.cs
...
public class HubOptionsSetup : IConfigureOptions<HubOptions>
{
    ...
    public HubOptionsSetup(IEnumerable<IHubProtocol> protocols)
    {
        ...
        {
            if (hubProtocol.GetType().CustomAttributes.Where(a => a.AttributeType.FullName == "EasyR.Internal.NonDefaultHubProtocolAttribute").Any())
            {
                continue;
            }
